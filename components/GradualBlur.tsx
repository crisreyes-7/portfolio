...
const PRESETS: Record<string, Partial<typeof DEFAULT_CONFIG & { width?: string }>> = {
...
function useResponsiveDimension(
  ...
  const [result, setResult] = useState(value ?? fallback);
  useEffect(() => {
    if (!responsive) return;
    const calc = () => setResult(value ?? fallback);
    const debounced = debounce(calc, 100);
    calc();
    window.addEventListener("resize", debounced);
    return () => window.removeEventListener("resize", debounced);
  }, [responsive, value, fallback]);
  return responsive ? result : (value ?? fallback);
}
...
function GradualBlur(props: GradualBlurProps) {
  ...
  const responsiveHeight = useResponsiveDimension(config.responsive, config.height, DEFAULT_CONFIG.height);
  const responsiveWidth = useResponsiveDimension(config.responsive, config.width, undefined);
